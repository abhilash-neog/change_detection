...
def train(configs, model, datamodule, callbacks=None):
...
def test(configs, model, datamodule, checkpoint_path, callbacks=None):
    tester = pl.Trainer.from_argparse_args(
        configs, callbacks=callbacks, benchmark=True
    )
