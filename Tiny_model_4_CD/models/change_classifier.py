...
import torchvision
from models.layers import MixingMaskAttentionBlock, PixelwiseLinear, UpMask, MixingBlock, BBoxRegressor, BoundingLinear
from torch import Tensor
from torch.nn import Module, ModuleList, Sigmoid
from torch.nn import Linear, Conv2d, PReLU, Sequential
...
class ChangeClassifier(Module):
    def __init__(
        ...
        # Bonding box regressor
        self._bboxHead = BoundingLinear()

    def forward(self, ref: Tensor, test: Tensor) -> Tensor:
        features = self._encode(ref, test)
        latents = self._decode(features)
        classification = self._classify(latents)
        boundingBoxes = self._bboxHead(classification)
